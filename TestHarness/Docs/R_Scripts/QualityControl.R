...
ja_OverlapMatrix <- function(x, y, DoCount = TRUE)
{
    ...
    for(i in 1:t)
    {
        for(j in i:t)
        {
            ...
            y1[j,i] = y1[i,j]
			perY[i,j] <- y1[i,j]/min(length(unique(xx1[,"Peptide"])),
				length(unique(xx2[,"Peptide"])))*100
			perY[j,i] <- perY[i,j]
        }
		y2 <- c(y2, y1[i,i])
    }
	...
	colnames(perY) <- 1:ncol(perY)
	rownames(y1) <- 1:nrow(y1)
	rownames(y1.1) <- 1:nrow(y1.1)
	rownames(perY) <- 1:nrow(perY)
...
ja_QCFractionsHeat <- function(zz, 
				file="DeleteMe.png",
				bkground="white", 
				IMGwidth=1200,
				IMGheight=1200,
